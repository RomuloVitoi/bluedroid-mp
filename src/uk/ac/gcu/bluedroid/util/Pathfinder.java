...
public class Pathfinder {
	...
	public static List<Node> generate(int startX, int startY, int endX, int endY, int[][] mapWalls) {
		return generate(toNode(startX, startY), toNode(endX, endY), mapWalls);
	}
	
	public static List<Node> generate(Node start, Node finish, int[][] mapWalls) {
	...
	private static List<Node> getNeighborNodes(Node n) {
		...
		if(walls[n.x + 1][n.y] != 1) found.add(toNode(n.x + 1, n.y));
		if(walls[n.x - 1][n.y] != 1) found.add(toNode(n.x - 1, n.y));
		if(walls[n.x][n.y + 1] != 1) found.add(toNode(n.x, n.y + 1));
		if(walls[n.x][n.y - 1] != 1) found.add(toNode(n.x, n.y - 1));
		if(canCutCorners) {
			if(walls[n.x + 1][n.y + 1] != 1 && (walls[n.x + 1][n.y] != 1 || walls[n.x][n.y + 1] != 1)) found.add(toNode(n.x + 1, n.y + 1));
			if(walls[n.x - 1][n.y + 1] != 1 && (walls[n.x - 1][n.y] != 1 || walls[n.x][n.y + 1] != 1)) found.add(toNode(n.x - 1, n.y + 1));
			if(walls[n.x - 1][n.y - 1] != 1 && (walls[n.x - 1][n.y] != 1 || walls[n.x][n.y - 1] != 1)) found.add(toNode(n.x - 1, n.y - 1));
			if(walls[n.x + 1][n.y - 1] != 1 && (walls[n.x + 1][n.y] != 1 || walls[n.x][n.y - 1] != 1)) found.add(toNode(n.x + 1, n.y - 1));
		}
